...
SeeAlso: #P0178,#P0187
 *
 * Version:	@(#)opti495.c	1.0.14	2021/03/16
 *
...
static uint8_t
opti495_in(uint16_t port, priv_t priv)
{
...
static void
opti495_out(uint16_t port, uint8_t val, priv_t priv)
{
    opti_t *dev = (opti_t *)priv;
...
opti_init(const device_t *info, UNUSED(void *parent))
{
    ...
    io_sethandler(0x0022, 1,
		  opti495_in,NULL,NULL, opti495_out,NULL,NULL, dev);
