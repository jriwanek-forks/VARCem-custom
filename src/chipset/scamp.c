...
static const struct {
    ...
    { 512,	0,	{ BANK_256K,             BANK_NONE		} },
    { 1024,	1,	{ BANK_256K_INTERLEAVED, BANK_NONE		} },
    { 1536,	2,	{ BANK_256K_INTERLEAVED, BANK_256K		} },
    { 2048,	3,	{ BANK_256K_INTERLEAVED, BANK_256K_INTERLEAVED	} },
    { 3072,	12,	{ BANK_256K_INTERLEAVED, BANK_1M		} },
    { 4096,	5,	{ BANK_1M_INTERLEAVED,   BANK_NONE		} },
    { 5120,	13,	{ BANK_256K_INTERLEAVED, BANK_1M_INTERLEAVED	} },
    { 6144,	6,	{ BANK_1M_INTERLEAVED,   BANK_1M		} },
    { 8192,	7,	{ BANK_1M_INTERLEAVED,   BANK_1M_INTERLEAVED	} },
    { 12288,	14,	{ BANK_1M_INTERLEAVED,   BANK_4M		} },
    { 16384,	9,	{ BANK_4M_INTERLEAVED,   BANK_NONE		} },
};
...
static const struct {
    ...
    /*Bank 2 remapped to 0*/
    { { BANK_4M,		BANK_NONE		}, 0	},
    { { BANK_4M_INTERLEAVED,	BANK_NONE		}, 0	},
      /*Bank 2 remapped to 0*/
    { { BANK_NONE,		BANK_4M			}, 1	},
      /*Banks 2/3 remapped to 0/1*/
    { { BANK_NONE,		BANK_4M_INTERLEAVED	}, 1	},

    { { BANK_256K_INTERLEAVED,	BANK_1M			}, 0	},
    { { BANK_256K_INTERLEAVED,	BANK_1M_INTERLEAVED	}, 0	},
    { { BANK_1M_INTERLEAVED,	BANK_4M			}, 0	},
      /*Undocumented - probably wrong!*/
    { { BANK_1M_INTERLEAVED,	BANK_4M_INTERLEAVED	}, 0	},
};
...
/*
 * Read/write handlers for interleaved memory banks.
 * We must keep CPU and RAM array mapping linear, otherwise we
 * won't be able to execute code from interleaved banks.
 */
static uint8_t 
...
recalc_mappings(priv_t priv)
{
    ...
    mem_set_mem_state((1 << 20), (16256 - 1024) * 1024,
		      MEM_READ_EXTERNAL | MEM_WRITE_EXTERNAL);
    mem_set_mem_state(0xfe0000, 0x20000,
		      MEM_READ_EXTERNAL | MEM_WRITE_EXTERNAL);
	...
	if (mem_size > 1024)
		mem_set_mem_state((1 << 20), (mem_size - 1024) << 10,
...
static uint8_t 
scamp_in(uint16_t port, priv_t priv)
{
    uint8_t ret = 0xff;
    scamp_t *dev = (scamp_t *) priv;
...
static void 
scamp_out(uint16_t port, uint8_t val, priv_t priv)
{
    scamp_t *dev = (scamp_t *) priv;
...
scamp_init(UNUSED(const device_t *info), UNUSED(void *parent))
{
    ...
    io_sethandler(0x00e8, 1,
		  scamp_in,NULL,NULL, scamp_out,NULL,NULL, dev);
    io_sethandler(0x00ea, 6,
		  scamp_in,NULL,NULL, scamp_out,NULL,NULL, dev);
    io_sethandler(0x00f4, 2,
		  scamp_in,NULL,NULL, scamp_out,NULL,NULL, dev);
    io_sethandler(0x00f9, 1,
		  scamp_in,NULL,NULL, scamp_out,NULL,NULL, dev);
    io_sethandler(0x00fb, 1,
		  scamp_in,NULL,NULL, scamp_out,NULL,NULL, dev);	
	...
	mem_map_add(&dev->ram_mapping[c], 0, 0,
		    ram_mirrored_read, NULL, NULL,
		    ram_mirrored_write, NULL, NULL,
		    &ram[addr], MEM_MAPPING_INTERNAL,
		    (void *)&dev->ram_struct[c]);
	mem_map_disable(&dev->ram_mapping[c]);
