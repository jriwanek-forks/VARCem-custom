 ...
 *
 * Version:	@(#)sio_f82c710.c	1.0.1	2021/03/16
 *
...
typedef struct upc_t {
    /* state of algorithm to enter configuration mode */
    int configuration_state;
    int configuration_mode;

    // next expected value of configuration algorithm
    uint8_t next_value;

    /* cri = configuration index register, addr is even */
    uint16_t cri_addr;

    /* cap = configuration access port, addr is odd and is cri_addr + 1 */
    uint16_t cap_addr;

    /* currently indexed register */
    uint8_t cri;

    /*
     * These regs are not affected by reset:
     *
     * there are 16 indexes, but there is no need to store the
     * last one which is: R = cri_addr / 4, W = exit config mode
     */
    uint8_t	regs[15];
    ...
    void	(*mouse_write)(uint8_t val, priv_t);
...
static uint8_t 
upc_in(uint16_t port, priv_t priv)
{
...
static void 
upc_out(uint16_t port, uint8_t val, priv_t priv)
{
    upc_t *dev = (upc_t *)priv; 
		...
		if (dev->configuration_state == 0) {
			...
			if (addr_verify == 0xff) {
				dev->configuration_mode = 1;
		    		...
		    		io_sethandler(dev->cri_addr, 2,
					      upc_in,NULL,NULL, upc_out,NULL,NULL, dev);
			} else {
	...
	if (port == dev->cri_addr) {
		...
		if (dev->cri == 0xf) {
			dev->configuration_mode = 0;
			io_removehandler(dev->cri_addr, 2,
					 upc_in,NULL,NULL, upc_out,NULL,NULL, dev);
		} else {
...
uint8_t
upc_mouse_in(uint16_t port, priv_t priv)
{
    upc_t *dev = (upc_t *)priv; 
    uint8_t ret = 0xff;
...
void
upc_mouse_out(uint16_t port, uint8_t val, priv_t priv)
{
    upc_t *dev = (upc_t *)priv;
...
void
upc_mouse_disable(priv_t priv)
{
    upc_t *dev = (upc_t *)priv;

    io_removehandler(dev->mdata_addr, 2,
		     upc_mouse_in,NULL,NULL, upc_mouse_out,NULL,NULL, dev);
}


void
upc_mouse_enable(priv_t priv)
{
    upc_t *dev = (upc_t *)priv;

    io_sethandler(dev->mdata_addr, 2,
		  upc_mouse_in,NULL,NULL, upc_mouse_out,NULL,NULL, dev);
}


void
upc_set_mouse(void (*mouse_write)(uint8_t val, priv_t), priv_t priv)
{
    upc_t *dev = (upc_t *)priv;
