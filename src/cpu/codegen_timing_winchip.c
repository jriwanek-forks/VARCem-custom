 ...
 *
 * Version:	@(#)codegen_timing_winchip.c	1.0.4	2021/11/02
 *
...
static void codegen_timing_winchip_prefix(uint8_t prefix, UNUSED(uint32_t fetchdat))
{
        timing_count += COUNT(opcode_timings[prefix], 0);
        last_prefix = prefix;
}

static void codegen_timing_winchip_opcode(uint8_t opcode, uint32_t fetchdat, int op_32, uint32_t op_pc)
{
        const int **timings;
        uint64_t *deps;
