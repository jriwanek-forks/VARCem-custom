 ...
 *
 *		Instruction timing for P6 (Pentium Pro+) 686 processors.
 *
 *
 * Version:	@(#)codegen_timing_p6.c	1.0.1	2021/10/29
 *
...
static int uop_run(const p6_uop_t *uop, int decode_time)
{
    ...
    if ((cpu_get_type() == CPU_PENTIUM_PRO) && uop->type == UOP_ALU0_SEG) {
...
//    DEBUG("ins->nr_uops=%i decode_buffer.nr_uops=%i decode_buffer.nr_ins=%i instr_length=%i\n", ins->nr_uops, decode_buffer.nr_uops, decode_buffer.nr_ins, instr_length);
