...
#include <86box/scsi_ncr53c810.h>
...
ncr53c810_soft_reset(ncr53c810_t *dev)
{
    ...
    /* Narrow-SCSI, so 8 targets with 8 lun's each. */
    for (i = 0; i < 8; i++) {
	for (j = 0; j < 8; j++)
...
static void
do_irq(ncr53c810_t *dev, int level)
{
    if (level) {
...
ncr53c810_do_dma(ncr53c810_t *dev, int out, uint8_t id)
{
    ...
    sd = &scsi_devices[id][dev->current_lun];
...
ncr53c810_do_command(ncr53c810_t *dev, uint8_t id)
{
    ...
    dev->current = (ncr53c810_request_t *)mem_alloc(sizeof(ncr53c810_request_t));
    dev->current->tag = id;
	...
	sd->cmd_buffer = (uint8_t *)mem_alloc(sd->buffer_length);
		...
		if (opcode < 5) {
			...
			switch (opcode) {
				case 0: /* Select */
					...
					if (! scsi_device_present(&scsi_devices[id][0])) {
						ncr53c810_bad_selection(dev, id);
						break;
					}
