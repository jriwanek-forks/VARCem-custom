 ...
 *
 * Version:	@(#)fdd_mfm.c	1.0.3	2019/04/07
 *
...
get_raw_size(int drive, int side)
{
    ...
    if (track_index == -1) {
	DEBUG("MFM: unable to find track(%i, %i)\n", dev->cur_track, side);
...
mfm_load(int drive, const wchar_t *fn)
{
    ...
    if (dev->f == NULL) {
	free(dev);
	memset(floppyfns[drive], 0x00, sizeof(floppyfns[drive]));
	return(0);
    }
    ...
    if (dev->hdr.if_type & 0x80) {
	dev->adv_tracks = (mfm_adv_track_t *)mem_alloc(dev->total_tracks * sizeof(mfm_adv_track_t));
	fread(dev->adv_tracks, 1, dev->total_tracks * sizeof(mfm_adv_track_t), dev->f);
    } else {
	dev->tracks = (mfm_track_t *)mem_alloc(dev->total_tracks * sizeof(mfm_track_t));
