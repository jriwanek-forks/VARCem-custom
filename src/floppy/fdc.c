...
static void call_back(priv_t priv);
 ...
 *  3 = 250 kbps
 *
 * Drive is only aware of selected rate and densel, so on real hardware, the
 * rate expected by fdc_t and the rate actually being processed by drive
 * can mismatch, in which case the fdc_t won't receive the correct data.
 */
void
...
static void
io_command_phase1(fdc_t *fdc, int out)
{
    fifo_buf_reset(fdc);
...
static void
fdc_out(uint16_t port, uint8_t val, priv_t priv)
{
    fdc_t *fdc = (fdc_t *) priv;
    int drive, i, drive_num;

    DBGLOG(2, "FDC: out(%04X, %02X)\n", port, val);
...
//FIXME: see above- there, too?  --FvK
			...
			timer_process();
			timer_update_outstanding();
			/* We simplify this since each motor now spins separately. */
			for (i = 0; i < FDD_NUM; i++) {
...
uint8_t
fdc_in(uint16_t addr, priv_t priv)
{
    fdc_t *fdc = (fdc_t *)priv;
    uint8_t ret = 0xff;
    int drive;
	...
	case 3:
		...
		if (fdc->flags & FDC_FLAG_PS1) {
			...
			/* PS/1 Model 2121 seems to return drive type in port
			 ...
			 *	      10=2.88M 3.5
			 *	      20=1.2M 5.25
			 *	      30=360K 5.25 ? sometimes works...
			 *
			 * as reported by Configur.exe.
			 */
			if (fdd_is_525(drive)) {
				ret = 0x20;		/* 5.25 360/1.2 */
			} else if (fdd_is_ed(drive))
				ret = 0x10;		/* 3.5 2.88M ED */
			  else
				ret = 0x00;		/* 3.5 1.44M HD */
...
static void
common_finish(fdc_t *fdc, int compare, int st5)
{
    fdc_int(fdc);
...
static void
readwrite_finish(fdc_t *fdc, int compare)
{
    fdc->inread = 0;
    fdc->interrupt = -2;

    common_finish(fdc, compare, 0);
}


static void
no_dma_end(fdc_t *fdc, int compare)
{
    fdc->time = 0LL;

    common_finish(fdc, compare, 0x80);
}
...
/*
 * Abnormal termination with both status 1 and 2 set to 0, used
 * when abnormally terminating the fdc_t FORMAT TRACK command.
 */
void
...
fdc_set_base(fdc_t *fdc, int base)
{
	...
	io_sethandler(base + (super_io ? 2 : 0), super_io ? 4 : 6,
		      fdc_in,NULL,NULL, fdc_out,NULL,NULL, fdc);
	io_sethandler(base+7, 1,
		      fdc_in,NULL,NULL, fdc_out,NULL,NULL, fdc);
    } else {
	io_sethandler(base+2, 1, NULL,NULL,NULL, fdc_out,NULL,NULL, fdc);
	io_sethandler(base+4, 1, fdc_in,NULL,NULL, NULL,NULL,NULL, fdc);
	io_sethandler(base+5, 1, fdc_in,NULL,NULL, fdc_out,NULL,NULL, fdc);
	if (fdc->flags & FDC_FLAG_TOSHIBA)
		io_sethandler(base+7, 1, fdc_in,NULL,NULL, fdc_out,NULL,NULL, fdc);
    }
...
fdc_remove(fdc_t *fdc)
{
	...
	io_removehandler(fdc->base_address+2, 1,
			 NULL,NULL,NULL, fdc_out,NULL,NULL, fdc);
	io_removehandler(fdc->base_address+4, 1,
			 fdc_in,NULL,NULL, NULL,NULL,NULL, fdc);
...
fdc_init(const device_t *info, UNUSED(void *parent))
{
    ...
    timer_add(call_back, fdc, &fdc->time, &fdc->time);
