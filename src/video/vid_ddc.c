 ...
 *
 * Version:	@(#)vid_ddc.c	1.0.3	2021/10/12
 *
...
ddc_init(void *i2c)
{
    ...
    edid->mfg[0] = 0x58; /* manufacturer "VAM" (apparently unassigned by UEFI Forum) */
    edid->mfg[1] = 0x2D;
    edid->mfg_week = 48;
    ...
    edid->features = 0xeb; /* DPMS standby/suspend/active-off; first timing is preferred; GTF/CVT */
...
/* 60 Hz timings */
    ...
    memcpy(&edid->descriptors[3].ascii, "VARCm Monitor", 13); /* exactly 13 characters (would otherwise require LF termination and space padding) */
