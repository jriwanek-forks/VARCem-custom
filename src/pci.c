		...
		if (! pci_bus) {
			...
			if (slot != 0xff) {
				if (pci_cards[slot].write) {
					DBGLOG(1, "PCI: writing card on slot %02X (pci_cards[%i])...\n", pci_card, slot);
		...
		if (! pci_bus) {
			...
			if (slot != 0xff) {
				if (pci_cards[slot].read) {
					DBGLOG(1, "PCI: reading from card on slot %02X (pci_cards[%i]) (%02X:%02X)...\n", pci_card, slot, pci_func, pci_index);
...
pci_set_mirq(uint8_t mirq)
{
	...
	DEBUG("pci_set_mirq(%02X): MIRQ0 disabled\n", mirq);
	return;
    }

    if (pci_mirqs[mirq].irq_line > 0x0f) {
	DEBUG("pci_set_mirq(%02X): IRQ line is disabled\n", mirq);
	return;
    }

    irq_line = pci_mirqs[mirq].irq_line;
    DEBUG("pci_set_mirq(%02X): Using IRQ %i\n", mirq, irq_line);

    if (pci_irq_is_level(irq_line) &&
	(pci_irq_hold[irq_line] & (1ULL << (0x1E + mirq)))) {
	/* IRQ already held, do nothing. */
	DEBUG("pci_set_mirq(%02X): MIRQ is already holding the IRQ\n", mirq);
	return;
    }
    DEBUG("pci_set_mirq(%02X): MIRQ not yet holding the IRQ\n", mirq);

    level = pci_irq_is_level(irq_line);
    if (!level || !pci_irq_hold[irq_line]) {
	DEBUG("pci_set_mirq(%02X): Issuing %s-triggered IRQ (%sheld)\n", mirq, level ? "level" : "edge", pci_irq_hold[irq_line] ? "" : "not ");
	...
	DEBUG("pci_set_mirq(%02X): IRQ line already being held\n", mirq);
    }

    /* If the IRQ is level-triggered, mark that this MIRQ is holding it. */
    if (level) {
	DEBUG("pci_set_mirq(%02X): Marking that this card is holding the IRQ\n", mirq);
	pci_irq_hold[irq_line] |= (1ULL << (0x1E + mirq));
    }

    DEBUG("pci_set_mirq(%02X): Edge-triggered interrupt, not marking\n", mirq);
}
...
pci_set_irq(uint8_t card, uint8_t pci_int)
{
    ...
    if (! last_pci_card) {
	DEBUG("pci_set_irq(%02X, %02X): No PCI slots (how are we even here?!)\n", card, pci_int);
	return;
    }
    DEBUG("pci_set_irq(%02X, %02X): %i PCI slots\n", card, pci_int, last_pci_card);

    slot = pci_card_to_slot_mapping[card];
    if (slot == 0xff) {
	DEBUG("pci_set_irq(%02X, %02X): Card is not on a PCI slot (how are we even here?!)\n", card, pci_int);
	return;
    }
    DEBUG("pci_set_irq(%02X, %02X): Card is on PCI slot %02X\n", card, pci_int, slot);

    if (! pci_cards[slot].irq_routing[pci_int_index]) {
	DEBUG("pci_set_irq(%02X, %02X): No IRQ routing for this slot and INT pin combination\n", card, pci_int);
	return;
    }

    irq_routing = (pci_cards[slot].irq_routing[pci_int_index] - PCI_INTA) & 3;
    DEBUG("pci_set_irq(%02X, %02X): IRQ routing for this slot and INT pin combination: %02X\n", card, pci_int, irq_routing);

    if (pci_irqs[irq_routing] > 0x0f) {
	DEBUG("pci_set_irq(%02X, %02X): IRQ line is disabled\n", card, pci_int);
	return;
    }

    irq_line = pci_irqs[irq_routing];
    DEBUG("pci_set_irq(%02X, %02X): Using IRQ %i\n", card, pci_int, irq_line);

    if (pci_irq_is_level(irq_line) && (pci_irq_hold[irq_line] & (1ULL << card))) {
	/* IRQ already held, do nothing. */
	DEBUG("pci_set_irq(%02X, %02X): Card is already holding the IRQ\n", card, pci_int);
	return;
    }
    DEBUG("pci_set_irq(%02X, %02X): Card not yet holding the IRQ\n", card, pci_int);

    level = pci_irq_is_level(irq_line);
    if (!level || !pci_irq_hold[irq_line]) {
	DEBUG("pci_set_irq(%02X, %02X): Issuing %s-triggered IRQ (%sheld)\n", card, pci_int, level ? "level" : "edge", pci_irq_hold[irq_line] ? "" : "not ");

	/* Only raise the interrupt if it's edge-triggered or level-triggered and not yet being held. */
	picintlevel(1 << irq_line);
    } else if (level && pci_irq_hold[irq_line]) {
	DEBUG("pci_set_irq(%02X, %02X): IRQ line already being held\n", card, pci_int);
    }

    /* If the IRQ is level-triggered, mark that this card is holding it. */
    if (pci_irq_is_level(irq_line)) {
	DEBUG("pci_set_irq(%02X, %02X): Marking that this card is holding the IRQ\n", card, pci_int);
	pci_irq_hold[irq_line] |= (1ULL << card);
    } else {
	DEBUG("pci_set_irq(%02X, %02X): Edge-triggered interrupt, not marking\n", card, pci_int);
    }
}
...
pci_clear_mirq(uint8_t mirq)
{
    ...
    if (mirq > 1) {
	DEBUG("pci_clear_mirq(%02X): Invalid MIRQ\n", mirq);
	return;
    }

    if (! pci_mirqs[mirq].enabled) {
	DEBUG("pci_clear_mirq(%02X): MIRQ0 disabled\n", mirq);
	return;
    }

    if (pci_mirqs[mirq].irq_line > 0x0f) {
	DEBUG("pci_clear_mirq(%02X): IRQ line is disabled\n", mirq);
	return;
    }

    irq_line = pci_mirqs[mirq].irq_line;
    DEBUG("pci_clear_mirq(%02X): Using IRQ %i\n", mirq, irq_line);

    if (pci_irq_is_level(irq_line) &&
	!(pci_irq_hold[irq_line] & (1ULL << (0x1E + mirq)))) {
	/* IRQ not held, do nothing. */
	DEBUG("pci_clear_mirq(%02X): MIRQ is not holding the IRQ\n", mirq);
	return;
    }

    level = pci_irq_is_level(irq_line);
    if (level) {
	DEBUG("pci_clear_mirq(%02X): Releasing this MIRQ's hold on the IRQ\n", mirq);
	pci_irq_hold[irq_line] &= ~(1 << (0x1E + mirq));

	if (! pci_irq_hold[irq_line]) {
		DEBUG("pci_clear_mirq(%02X): IRQ no longer held by any card, clearing it\n", mirq);
       		picintc(1 << irq_line);
	} else {
		DEBUG("pci_clear_mirq(%02X): IRQ is still being held\n", mirq);
	}
    } else {
	DEBUG("pci_clear_mirq(%02X): Clearing edge-triggered interrupt\n", mirq);
	picintc(1 << irq_line);
   }
}
...
pci_clear_irq(uint8_t card, uint8_t pci_int)
{
    ...
    if (! last_pci_card) {
	DEBUG("pci_clear_irq(%02X, %02X): No PCI slots (how are we even here?!)\n", card, pci_int);
	return;
    }
    DEBUG("pci_clear_irq(%02X, %02X): %i PCI slots\n", card, pci_int, last_pci_card);

    slot = pci_card_to_slot_mapping[card];
    if (slot == 0xff) {
	DEBUG("pci_clear_irq(%02X, %02X): Card is not on a PCI slot (how are we even here?!)\n", card, pci_int);
	return;
    }
    DEBUG("pci_clear_irq(%02X, %02X): Card is on PCI slot %02X\n", card, pci_int, slot);

    if (! pci_cards[slot].irq_routing[pci_int_index]) {
	DEBUG("pci_clear_irq(%02X, %02X): No IRQ routing for this slot and INT pin combination\n", card, pci_int);
	return;
    }

    irq_routing = (pci_cards[slot].irq_routing[pci_int_index] - PCI_INTA) & 3;
    DEBUG("pci_clear_irq(%02X, %02X): IRQ routing for this slot and INT pin combination: %02X\n", card, pci_int, irq_routing);

    if (pci_irqs[irq_routing] > 0x0f) {
	DEBUG("pci_clear_irq(%02X, %02X): IRQ line is disabled\n", card, pci_int);
	return;
    }

    irq_line = pci_irqs[irq_routing];
    DEBUG("pci_clear_irq(%02X, %02X): Using IRQ %i\n", card, pci_int, irq_line);

    if (pci_irq_is_level(irq_line) &&
	!(pci_irq_hold[irq_line] & (1ULL << card))) {
	/* IRQ not held, do nothing. */
	DEBUG("pci_clear_irq(%02X, %02X): Card is not holding the IRQ\n", card, pci_int);
	return;
    }

    level = pci_irq_is_level(irq_line);
    if (level) {
	DEBUG("pci_clear_irq(%02X, %02X): Releasing this card's hold on the IRQ\n", card, pci_int);
	pci_irq_hold[irq_line] &= ~(1 << card);

	if (! pci_irq_hold[irq_line]) {
		DEBUG("pci_clear_irq(%02X, %02X): IRQ no longer held by any card, clearing it\n", card, pci_int);
       		picintc(1 << irq_line);
	} else {
		DEBUG("pci_clear_irq(%02X, %02X): IRQ is still being held\n", card, pci_int);
	}
    } else {
	DEBUG("pci_clear_irq(%02X, %02X): Clearing edge-triggered interrupt\n", card, pci_int);
	picintc(1 << irq_line);
    }
}
...
pci_register_slot(int card, int type, int inta, int intb, int intc, int intd)
{
    ...
    DEBUG("pci_register_slot(): pci_cards[%i].id = %02X\n", last_pci_card, card);
...
pci_add_card(uint8_t add_type, uint8_t (*read)(int, int, priv_t), void (*write)(int, int, uint8_t, priv_t), priv_t priv)
{
	...
	DEBUG("pci_add_card(): Adding PCI CARD at specific slot %02X [SPECIFIC]\n", add_type);

    if (! PCI) {
	ERRLOG("pci_add_card(): Adding PCI CARD failed (non-PCI machine) [%s]\n", (add_type == PCI_ADD_NORMAL) ? "NORMAL" : ((add_type == PCI_ADD_VIDEO) ? "VIDEO" : "SPECIFIC"));
	return 0xff;
    }

    if (! last_pci_card) {
	ERRLOG("pci_add_card(): Adding PCI CARD failed (no PCI slots) [%s]\n", (add_type == PCI_ADD_NORMAL) ? "NORMAL" : ((add_type == PCI_ADD_VIDEO) ? "VIDEO" : "SPECIFIC"));
	return 0xff;
    }
	...
	if (!dev->read && !dev->write) {
		if (((dev->type == PCI_CARD_NORMAL) && (add_type >= PCI_ADD_NORMAL)) ||
			...
			dev->write = write;
			dev->priv = priv;
			DEBUG("pci_add_card(): Adding PCI CARD to pci_cards[%i] (slot %02X) [%s]\n", i, dev->id, (add_type == PCI_ADD_NORMAL) ? "NORMAL" : ((add_type == PCI_ADD_VIDEO) ? "VIDEO" : "SPECIFIC"));
			return dev->id;
		}
	}
    }

    ERRLOG("pci_add_card(): Adding PCI CARD failed (unable to find a suitable PCI slot) [%s]\n", (add_type == PCI_ADD_NORMAL) ? "NORMAL" : ((add_type == PCI_ADD_VIDEO) ? "VIDEO" : "SPECIFIC"));
