...
ide_irq_raise(ide_t *ide)
{
    ...
    DBGLOG(1, "IDE: raising IRQ %i (board %i)\n", ide_boards[ide->board]->irq, ide->board);
...
ide_irq_lower(ide_t *ide)
{
    ...
    DBGLOG(1, "IDE: lowering IRQ %i (board %i)\n", ide_boards[ide->board]->irq, ide->board);
...
ide_identify(ide_t *ide)
{
    ...
    DEBUG("IDE IDENTIFY or IDENTIFY PACKET DEVICE on board %i (channel %i)\n", ide->board, ide->channel);
...
ide_set_sector(ide_t *ide, int64_t sector_num)
{
    ...
    if (ide->lba) {
	ide->head = (uint16_t)(sector_num >> 24);
	ide->cylinder = (uint32_t)(sector_num >> 8);
	ide->sector = (uint32_t)(sector_num);
    } else {
	cyl = (unsigned int)(sector_num / (hdd[ide->hdd_num].hpc * hdd[ide->hdd_num].spt));
...
ide_read_data(ide_t *ide, int length)
{
	...
	if ((ide->command == WIN_READ) || (ide->command == WIN_READ_NORETRY) || (ide->command == WIN_READ_MULTIPLE)) {
		...
		if (ide->secount) {
			...
			if (ide->command != WIN_READ_MULTIPLE)
				ui_sb_icon_update(SB_DISK | hdd[ide->hdd_num].bus, 0);
		}
	}
    }
...
ide_read_alt_status(uint16_t addr, priv_t priv)
{
    ...
    DEBUG("ide_read_alt_status(%04X, %08X) = %02X\n", addr, priv, temp);
    return temp;
}
...
ide_readw(uint16_t addr, priv_t priv)
{
    ...
    DBGLOG(1, "ide_readw(%04X, %08X) = %04X\n", addr, priv, temp);
    return temp;
}
...
ide_readl(uint16_t addr, priv_t priv)
{
    ...
    DBGLOG(1, "ide_readl(%04X, %08X) = %04X\n", addr, priv, temp);
    return temp;
}
