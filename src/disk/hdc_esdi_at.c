 ...
 *
 * Version:	@(#)hdc_esdi_at.c	1.0.20	2021/03/16
 *
...
typedef struct {
  ...
  uint8_t	command;
   ...
   int		pos;
    ...
    drive_t	drives[ESDI_NUM];

    rom_t	bios_rom;
} hdc_t;


static uint8_t	hdc_in(uint16_t port, priv_t priv);
static void	hdc_out(uint16_t port, uint8_t val, priv_t priv);
...
get_sector(hdc_t *dev, off64_t *addr)
{
    ...
    if (dev->head > heads) {
	DEBUG("ESDI: sector: past end of configured heads\n");
	return(0);
    }

    if (dev->sector >= sectors+1) {
	DEBUG("ESDI: sector: past end of configured sectors\n");
	return(0);
    }
...
call_back(priv_t priv)
{
	...
	case CMD_DIAGNOSE:
		/*
		 * This is basically controller diagnostics - it
		 * resets drive select to 0, and resets error and
		 * status to ready, DSC, and no error detected.
		 */
		dev->drive_sel = 0;
...
hdc_inw(uint16_t port, priv_t priv)
{
    ...
    if (port > 0x01f0) {
	temp = hdc_in(port, priv);
	if (port == 0x01f7)
		temp |= 0xff00;
	else
		temp |= (hdc_in(port + 1, priv) << 8);
    } else {
...
hdc_outw(uint16_t port, uint16_t val, priv_t priv)
{
    ...
    if (port > 0x01f0) {
	hdc_out(port, val & 0xff, priv);
	if (port != 0x01f7)
		hdc_out(port + 1, (val >> 8) & 0xff, priv);
    } else {
...
hdc_out(uint16_t port, uint8_t val, priv_t priv)
{
    ...
    DBGLOG(2, "WD1007 out(%04x, %02x)\n", port, val);

    switch (port) {
	case 0x1f0:	/* data */
		hdc_outw(port, val | (val << 8), priv);
		return;
...
wd1007vse1_init(const device_t *info, UNUSED(void *parent))
{
    ...
    mem_map_set_handler(&dev->bios_rom.mapping,
			rom_read,rom_readw,rom_readl, hdc_write,NULL,NULL);

    io_sethandler(0x01f0, 1,
		  hdc_in,hdc_inw,NULL, hdc_out,hdc_outw,NULL, dev);
    io_sethandler(0x01f1, 7,
		  hdc_in,hdc_inw,NULL, hdc_out,hdc_outw,NULL, dev);
    io_sethandler(0x03f6, 1, NULL,NULL,NULL, hdc_out,NULL,NULL, dev);
